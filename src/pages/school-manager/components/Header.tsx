...
export default function Header({ onMenuClick }: HeaderProps) {
  ...
  const displayName = profile?.full_name || 'School Manager';
  const initials = profile ? getInitials(profile.full_name) : 'SM';

  return (
    <header className="bg-white border-b border-gray-200 sticky top-0 z-40">
      <div className="px-3 md:px-6 lg:px-8 py-3 md:py-4 flex items-center justify-between gap-3">
        <div className="flex items-center gap-2 md:gap-3 flex-1 min-w-0">
          ...
          <div className="min-w-0">
            <h1 className="text-base md:text-xl font-bold text-gray-900 truncate">School Manager Portal</h1>
            <p className="text-xs md:text-sm text-gray-600 hidden sm:block truncate">Welcome back, {displayName.split(' ')[0]}</p>
          </div>
        </div>
          ...
          <div className="relative flex items-center gap-2 md:gap-3 pl-2 md:pl-4 border-l border-gray-200">
            ...
            >
              ...
              <div className="text-left hidden md:block">
                ...
                <p className="text-xs text-gray-600 truncate">{profile?.email || 'School Manager'}</p>
              </div>
