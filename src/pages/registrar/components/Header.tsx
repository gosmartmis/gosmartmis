...
export default function Header({ onMenuClick }: HeaderProps) {
  ...
  const displayName = profile?.full_name || 'Registrar';
  const initials = profile ? getInitials(profile.full_name) : 'R';
      ...
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-2 md:gap-3 flex-1 min-w-0">
          ...
          <div className="min-w-0 flex-1">
            <h1 className="text-base md:text-xl lg:text-2xl font-bold text-gray-900 truncate">
              Welcome Back, {displayName.split(' ')[0]}
            </h1>
            <p className="text-xs md:text-sm text-gray-600 hidden sm:block truncate">Manage student registrations and enrollment</p>
          </div>
        </div>

        <div className="flex items-center gap-2 md:gap-4 flex-shrink-0">
          <NotificationBell />
          <button className="hidden sm:flex relative p-2 text-gray-600 hover:bg-gray-100 rounded-lg transition-colors cursor-pointer">
          ...
          <div className="relative flex items-center gap-2 md:gap-3 pl-2 md:pl-4 border-l border-gray-200">
            ...
            >
              ...
              <div className="text-left hidden md:block">
                <div className="text-sm font-semibold text-gray-900 truncate max-w-[120px]">{displayName}</div>
                <div className="text-xs text-gray-500 truncate">{profile?.email || 'School Registrar'}</div>
              </div>
