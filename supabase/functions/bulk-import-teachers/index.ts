...
async function bulkGenerateRegNumbers(adminClient: any, schoolId: string, count: number) {
  const school = await fetchSchoolRegConfig(adminClient, schoolId)
  if (!school || !school.reg_number_prefix || school.reg_number_prefix.trim() === '') return []
  ...
  const startCounter = (school.reg_number_counter ?? 0) + 1
  await adminClient.from('schools').update({ reg_number_counter: (school.reg_number_counter ?? 0) + count }).eq('id', schoolId)
  return Array.from({ length: count }, (_, i) => {
    ...
    return { reg_number, synthetic_email: `${emailPart}@${slug}.gosmart` }
  })
}
...
serve(async (req) => {
  ...
  try {
    ...
    for (let i = 0; i < usersArray.length; i++) {
      ...
      let { email } = entry
      const rowNum = i + 1
      if (!full_name) { results.push({ row: rowNum, full_name: full_name || '', email: email || '', status: 'error', error: 'full_name is required' }); continue }
      const regEntry = regNumberPool[i] ?? null
      const regNumber = regEntry?.reg_number ?? null
      if (!email && isStudent && regEntry) email = regEntry.synthetic_email
      if (!email) { results.push({ row: rowNum, full_name, email: '', status: 'error', error: isStudent ? 'Provide email or configure registration number prefix' : 'email is required' }); continue }
      const cleanEmail = email.trim().toLowerCase()
      const cleanName = full_name.trim()

      try {
        const chars = 'ABCDEFGHJKMNPQRSTUVWXYZabcdefghjkmnpqrstuvwxyz23456789'
        const tempPassword = 'GoSmart@' + Array.from({ length: 8 }, () => chars[Math.floor(Math.random() * chars.length)]).join('')
        ...
        if (authError) {
          if (authError.message.includes('already been registered') || authError.message.includes('already exists')) {
            ...
            if (existing) {
              const profileData: Record<string, unknown> = { id: existing.id, school_id, full_name: cleanName, email: cleanEmail, phone: phone?.trim() || null, role: targetRole, is_active: true }
              if (regNumber) profileData.registration_number = regNumber
              await adminClient.from('profiles').upsert(profileData, { onConflict: 'id' })
            }
            results.push({ row: rowNum, full_name: cleanName, email: cleanEmail, registration_number: regNumber || undefined, status: 'exists' })
          } else {
        ...
        results.push({ row: rowNum, full_name: cleanName, email: cleanEmail, registration_number: regNumber || undefined, status: 'created', temp_password: tempPassword })
      } catch (err: any) {
